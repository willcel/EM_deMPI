    ...
    integer(kind=IB), parameter :: NP=60, itermax=5, strategy=6, &
    ...
    real *8 sigma1,sigma2,sigma3,sigma4  ! ��� ������ ���� ˮ
    ...
    Open (17, File='label.dat', Status='unknown')
    ...
    write(17,20)((true_model(I,J),J=1,npara),I=1,ns)
    ...
    do k=1,ns
        ...
        ! ��ǰʱ�̵Ĳ������絼��
        do i=1,npara
            if(i .le. nolayer)then  ! �絼�ʵĳ�ʼֵ�ͷ�Χ
...
20  Format(9E14.6)   ! ÿ�����Ĳ������� 2*nolayers-1
...
!    print*,'temp1 = ',1.d-02*temp1
    ...
    do while (iter <= itermax)
        ...
        rt=mod(rot+ind(4),NP)
        a5=a4(rt+1)
        bm_XC=spread(bestmemit_XC, DIM=1, NCOPIES=NP)  ! ����ά��dim����bestmemit_XC����NP��
        ...
        do i = 1,size(ui_XC_index)
            ...
            ui_XC(cur_i,:)=max(min(ui_XC(cur_i,:),XCmax),XCmin)
        ...
        ! call MPI_Bcast(pop_XC) ! ��ͬ���̶�ͬһ�������Ĳ�ͬλ�����޸ģ���Bcast������
        ! ���ԣ�����Ҫ��Gather��������Ϊ�ڴ�ɢ���ڸ�����
        ! call MPI_Bcast(val)
...
203 format(2x, 'No. of iteration  =', I8)
    ...
    do k=1,npls
        do i=(nt*(k-1)+1),(nt*k)
            if(k==1)then                         ! ��һ������
                ...
                t(i)=t(i-nt)+t_ed            ! ����ӵ�t_ed�Ǽ���t_ed��ʱ������
            end if
        end do
    end do
    ...
    Else If (ic==2) Then
        Do i = 1, size(t)
            ...
            ik = 0
            Do ip = 1, npls
                If (t(i)>(ip-1)*(tt1+tt2) .And. t(i)<ip*(tt1+tt2)) kpls = ip   ! �жϳ��ʱ��λ����һ�������
            End Do

            Do ip = 1,kpls-1

                Call frt(rho, hh, t(i)-(ip-1)*(tt1+tt2), s2, 2, zplus, zminus, 0, ik, nlayer)  ! ������Ӧ
